...
data_new <- read.csv("Bank_Seleccion_Variables.csv",sep=";")
...
library(caret)
nzv <- nearZeroVar(data_new,
                   saveMetrics= TRUE)
nzv

nzv$nzv

# Segun los resultados las variables con valor TRUE son:
#  "default", "pdays" 
#  las cuales tendremos que eliminar

data_new <- data_new[,-c(5,14)]
str(data_new)
...
library(caret)
dummies <- dummyVars(y ~ ., data=data_new,
                      fullRank = TRUE)
datos <- as.data.frame(predict(dummies, newdata=data_new))
datos <- cbind(datos,y = data_new$y)
str(datos)
...
#-------------------------------------------------------------------
# 5.1 Aplicando el algoritmo Naive Bayes
...
clase.pred <- predict(modelo_nb,newdata = imbal_testing )
head(clase.pred)

proba.pred <- predict(modelo_nb,newdata = imbal_testing, type="prob")
head(proba.pred)
...
library(gmodels)
CrossTable(x = imbal_testing$y, y = clase.pred,
           prop.t=FALSE, prop.c=FALSE, prop.chisq = FALSE)

result1 <- caret::confusionMatrix(clase.pred,
                                  imbal_testing$y,
                                  positive="yes")
...
#-------------------------------------------------------------------
# 2.2 Aplicando el modelo de Regresión Logística
#     con Validación Cruzada con el paquete caret
...
clase.pred <- predict(modelo_rl,newdata = imbal_testing )
head(clase.pred)

proba.pred <- predict(modelo_rl,newdata = imbal_testing, type="prob")
head(proba.pred)

proba.pred <- proba.pred[,2]
head(cbind(imbal_testing,proba.pred,clase.pred))
...
result2 <- caret::confusionMatrix(clase.pred,
                                  imbal_testing$y,
                                  positive="yes")
...
#-------------------------------------------------------------------
# 2.3 Aplicando el algoritmo k-nn
#     con Validación Cruzada con el paquete caret
...
clase.pred <- predict(modelo_knn,newdata = imbal_testing )
head(clase.pred)

proba.pred <- predict(modelo_knn,newdata = imbal_testing, type="prob")
head(proba.pred)
...
result3 <- caret::confusionMatrix(clase.pred,
                                  imbal_testing$y,
                                  positive="yes")
...
result1$byClass["Sensitivity"]   # Naive-Bayes
result2$byClass["Sensitivity"]   # Regresión Logística
result3$byClass["Sensitivity"]   # Naive-Bayes
